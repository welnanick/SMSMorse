...
public class EventReceiver extends BroadcastReceiver {
    ...
    //Character sets
    private static final String CHARSET_MORSE =
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(EventReceiver.VIBRATE_IN_MORSE)) {
            ...
            this.vibrateMorse(
                    this.convertToVibrations(extras.getString(EventReceiver.VIBRATE_IN_MORSE_KEY)));

        }
        else if (action.equals(EventReceiver.SMS_RECEIVED)) {

            final boolean smsValid = extras != null;
            final boolean enabled = this.settings
                    .getBoolean(this.resources.getString(R.string.preference_enabled),
                            EventReceiver.DEFAULT_ENABLED);
            final boolean keygaurdOn =
                    ((KeyguardManager) context.getSystemService(Context.KEYGUARD_SERVICE))
                            .inKeyguardRestrictedInputMode();
            final boolean screenOffOnly = this.settings
                    .getBoolean(this.resources.getString(R.string.preference_screen_off_only),
                            EventReceiver.DEFAULT_SCREEN_OFF_ONLY);

            final int audioMode = ((AudioManager) context.getSystemService(Context.AUDIO_SERVICE))
                    .getRingerMode();
            final boolean activeAudioMode = (((audioMode == AudioManager.RINGER_MODE_NORMAL) &&
                    this.settings.getBoolean(
                            this.resources.getString(R.string.preference_vibrate_normal),
                            EventReceiver.DEFAULT_VIBRATE_NORMAL)) ||
                    ((audioMode == AudioManager.RINGER_MODE_VIBRATE) && this.settings.getBoolean(
                            this.resources.getString(R.string.preference_vibrate_vibrate),
                            EventReceiver.DEFAULT_VIBRATE_VIBRATE)) ||
                    ((audioMode == AudioManager.RINGER_MODE_SILENT) && this.settings.getBoolean(
                            this.resources.getString(R.string.preference_vibrate_silent),
            ...
            if (smsValid && enabled && activeAudioMode && (keygaurdOn || !screenOffOnly)) {
                ...
                final int vibrateParts = Integer.parseInt(
                        settings.getString(resources.getString(R.string.preference_vibrate_parts),
                ...
                for (SmsMessage message : messages) {
                    ...
                    if ((vibrateParts == EventReceiver.VIBRATE_CONTENT_SENDER) ||
                        ...
                        vibrations.addAll(this.convertSenderToVibrations(context,
    ...
    private ArrayList<Long> convertSenderToVibrations(Context context, String sender) {
        ...
        if (this.settings
                .getBoolean(context.getString(R.string.preference_lookup_contact_name), true)) {
            ...
            final Cursor results = context.getContentResolver()
                                          .query(Contacts.Phones.CONTENT_URI, projection, selection,
                                                  null, Contacts.ContactMethods.PERSON_ID);

            if (results.moveToFirst()) {

                return this.convertToVibrations(results.getString(
                        results.getColumnIndex(Contacts.PeopleColumns.DISPLAY_NAME)));
    ...
    private ArrayList<Long> convertToVibrations(final String message, final boolean isNumber) {
        ...
        final boolean vibrateCounts = this.settings
                .getBoolean(this.resources.getString(R.string.preference_vibrate_counts),
                        EventReceiver.DEFAULT_VIBRATE_COUNTS);

        //Establish all lengths
        final long dot = this.settings
                .getInt(this.resources.getString(R.string.preference_dot_length),
        ...
        final boolean[][] lookups =
                isNumber && vibrateCounts ? EventReceiver.COUNTS : EventReceiver.MORSE;
